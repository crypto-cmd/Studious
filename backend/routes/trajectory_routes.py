...
def predict_final_grade(student_id, course_code):
    predicted_grade = db.table("courses").select("predicted_grades").eq("student_id", student_id).eq("code", course_code).execute().data[0]["predicted_grades"]
    
    grade_predictor = GradePredictor(predicted_grade)
    ...
    predicted_final_grade = grade_predictor.predict_final_grade(final_month)
    saveto_db = db.table("courses").update({"final_predicted_grade": predicted_final_grade}).eq("student_id", student_id).eq("code", course_code).execute()

...
function NavItem({ icon, id, isActive, onClick }: NavItemProps) {
    return (
        <button
            ...
            className={`group relative h-12 w-12 shrink-0 rounded-full flex items-center justify-center outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/60 transition-[transform,color] duration-500 [transition-timing-function:cubic-bezier(0.22,1.25,0.36,1)] active:scale-95 ${
                isActive
                    ? "text-cyan-200"
        ...
        >
            <span
                className={`absolute inset-0 rounded-full transition-[opacity,transform,background-color,box-shadow] duration-500 [transition-timing-function:cubic-bezier(0.22,1.25,0.36,1)] ${
                    isActive
            ...
            {React.cloneElement(icon, {
                className: `relative z-10 w-6 h-6 transition-transform duration-500 [transition-timing-function:cubic-bezier(0.22,1.25,0.36,1)] ${
                    isActive ? "scale-110 nav-icon-active" : "group-hover:scale-110"
                }`,
            })}
        </button>
    );
